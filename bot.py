...
QUESTIONS_BY_GROUP = {
    ...
    'myself': [
        ...
        "🕰️ Моя лучшая черта характера? 🕰️",
        "💎 Моё самое большое достижение? 💎"
    ],
    'family': [
        ...
        "👵 Моя любимая бабушка? 👵",
        "📸 Люблю ли я семейные фото? 📸",
        "🏠 Хочу ли я жить отдельно? 🏠"
    ],
    'school': [
    ...
    'social': [
        ...
        "🔒 Мой аккаунт приватный или открытый? 🔒",
        "💬 Сколько времени сижу в телеграме? 💬"
    ],
    'dreams': [
        ...
        "💍 Какой я представляю свою свадьбу? 💍"
    ],
    'kpop': [
        ...
        "🫶 Кто мой вайб? 🫶",
        "🎁 Какой мерч я хочу? 🎁",
        "📺 Моё любимое k-pop шоу? 📺",
        "🌙 Какой юнит или соло я люблю? 🌙"
    ],
    'animals': [
        ...
        "🦁 Что делаю при виде бездомного животного? 🦁",
        "🐭 Было ли у меня животное в детстве? 🐭",
        "🐬 Какое животное мне кажется самым умным? 🐬",
        "🐘 Хочу ли я поехать на сафари? 🐘",
        "🐧 Какое животное отражает мой характер? 🐧"
    ],
    'travel': [
        "✈️ Куда я мечтаю поехать? ✈️",
        "🏖️ Мой идеальный отдых? 🏖️",
        "🎒 Что я беру в поездку? 🎒",
        "🚗 Как я добираюсь до места? 🚗",
        "📸 Что я делаю в дороге? 📸",
        "🌍 Где я уже была? 🌍",
        "👯 С кем я хочу путешествовать? 👯",
        "🗺️ Что делаю, если потерялась? 🗺️",
        "🏨 Отель или хостел? 🏨",
        "🍜 Какая национальная кухня нравится? 🍜"
    ],
    'food': [
        "🍕 Моё любимое блюдо? 🍕",
        "😖 Что я ненавижу есть? 😖",
    ...
    'games': [
        ...
        "🎯 Мой ранг в любимой игре? 🎯",
        "🕹️ С кем я играю? 🕹️",
        "💰 Трачу ли я деньги на игры? 💰",
        "🎁 Какой скин хочу? 🎁",
        "🏆 Моё достижение в игре? 🏆",
        "💻 Играю на телефоне или компьютере? 💻",
        "🎮 Как часто я играю? 🎮"
    ],
    'movies': [
        "🎬 Мой любимый фильм? 🎬",
        "🍿 Какой жанр я люблю? 🍿",
        "🎭 Мой любимый актёр/актриса? 🎭",
        "📺 Какой сериал я смотрю? 📺",
        "😭 Какой фильм заставил меня плакать? 😭",
        "😂 Какой фильм меня рассмешил? 😂",
        "🎥 В кино или дома? 🎥",
        "🍿 С кем я смотрю фильмы? 🍿",
        "🎬 Какой фильм пересматривала много раз? 🎬",
        "📽️ Какой фильм жду с нетерпением? 📽️"
    ],
    'sport': [
        "⚽ Каким спортом я занимаюсь? ⚽",
        "🏆 Моя любимая команда? 🏆",
        "🏅 За кого болею? 🏅",
        "🏃‍♀️ Как часто я тренируюсь? 🏃‍♀️",
        "🧘‍♀️ Йога или фитнес? 🧘‍♀️",
        "🎾 Смотрю ли я спортивные соревнования? 🎾",
        "🏊‍♀️ Умею ли я плавать? 🏊‍♀️",
        "🚴‍♀️ Люблю ли велосипед? 🚴‍♀️",
        "🥇 Моё спортивное достижение? 🥇",
        "🏋️‍♀️ Хожу ли в зал? 🏋️‍♀️"
    ]
}
...
def can_save_test(user_id, test_id):
    """Проверяет, можно ли сохранить тест"""
    conn = get_db()
    try:
        c = conn.cursor()
        
        # Проверяем, не свой ли это тест
        c.execute('SELECT creator_id FROM tests WHERE id = ?', (test_id,))
        row = c.fetchone()
        if row and row['creator_id'] == user_id:
            return False  # Нельзя сохранять свой тест
        
        # Проверяем лимит сохранённых
...
def save_test(user_id, test_id):
    """Сохраняет тест (если он не свой)"""
    conn = get_db()
    try:
        # Проверяем, не свой ли тест
        c = conn.cursor()
        c.execute('SELECT creator_id FROM tests WHERE id = ?', (test_id,))
        row = c.fetchone()
        if row and row['creator_id'] == user_id:
...
def get_saved_tests_count(user_id):
    """Возвращает количество сохранённых тестов (исключая свои)"""
    conn = get_db()
    try:
        c = conn.cursor()
        c.execute('''
...
async def send_test_completed_notification(bot, creator_id, friend_name, test_title, score):
    try:
        ...
        await safe_send_message(bot, creator_id, text, parse_mode=ParseMode.MARKDOWN)
    except Exception as e:
...
def get_test_action_keyboard(test_id, is_owner=False, is_saved=False):
    ...
    # Кнопка "Поделиться" только для своих тестов
    if is_owner:
...
async def send_referral_notification(bot, referrer_id, new_user_name, new_user_username=None, is_new_user=True):
    """Отправляет уведомление о переходе по реферальной ссылке"""
    try:
        ...
        # Формируем имя с username если есть
...
async def select_correct_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"Сохранён вопрос {data['current_q']}/{data['total_q']}")
...
async def finish_creation(query_or_update, context, user_id):
    data = context.user_data.get('create_test', {})
    if not data:
        logger.error("Нет данных create_test в finish_creation")
        return
    
    logger.info(f"Завершение создания теста. Вопросов: {len(data.get('questions_data', []))}")
    ...
    if not use_attempt(user_id):
        ...
        if hasattr(query_or_update, 'message'):
            await query_or_update.message.reply_text(msg, parse_mode=ParseMode.MARKDOWN, reply_markup=get_main_keyboard(user_id))
        else:
            await query_or_update.message.reply_text(msg, parse_mode=ParseMode.MARKDOWN, reply_markup=get_main_keyboard(user_id))
        del context.user_data['create_test']
        return
    ...
    logger.info(f"Создаём тест: {data['title']}, вопросов: {len(questions)}")
...
async def save_after_create(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if save_test(user_id, test_id):
        await query.message.reply_text("⭐ *Тест сохранён в «Мои тесты»!* ⭐", parse_mode=ParseMode.MARKDOWN, reply_markup=get_main_keyboard(user_id))
    else:
        await query.message.reply_text("💔 *Тест уже сохранён!* 💔", parse_mode=ParseMode.MARKDOWN, reply_markup=get_main_keyboard(user_id))
...
async def top_friends_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, friend in enumerate(friends, 1):
        ...
        # Для премиум-пользователей добавляем кнопку "Детальные ответы"
        if has_premium:
            text += f"   👆 Нажми на кнопку ниже, чтобы увидеть ответы\n"
            keyboard.append([
                InlineKeyboardButton(
                    f"📊 Ответы {name}", 
                    callback_data=f"friend_details_{friend['friend_id']}"
                )
            ])
...
async def show_friend_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_premium(user_id):
        ...
        # Отправляем сообщение с предложением купить премиум
        await query.message.reply_text(
            "💎✨ *ТОЛЬКО ДЛЯ ПРЕМИУМ!* ✨💎\n\n"
            "🌸 *Хочешь увидеть, как подружки ответили на твои вопросы?*\n\n"
            "💫 *С ПРЕМИУМ ты сможешь:*\n"
            "• 📋 Смотреть детальные ответы каждой подружки\n"
            "• ❌ Видеть, где они ошиблись\n"
            "• ✅ Сравнивать их ответы с правильными\n"
            "• 📊 Анализировать, кто знает тебя лучше всех\n\n"
            "🎁 *ВСЕГО ЗА 199₽ В МЕСЯЦ!*\n\n"
    ...
    if not answers_details:
        await query.message.edit_text(
            ...
            "Подружка ещё не проходила твои тесты или данные устарели.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🔙 Назад", callback_data="top_friends")
            ]])
    ...
    for test_idx, test in enumerate(answers_details[:3]):  # Показываем только 3 последних теста
        ...
        for i, q in enumerate(test['questions'][:5]):  # Показываем первые 5 вопросов
    ...
    if len(answers_details) > 3:
        text += f"\n✨ *И ещё {len(answers_details) - 3} пройденных тестов!* ✨"
    
    # Создаём клавиатуру с навигацией
    keyboard = []
    
    # Если есть другие друзья, добавляем кнопку возврата к списку
    keyboard.append([InlineKeyboardButton("🔙 Назад к списку подруг", callback_data="top_friends")])
    keyboard.append([InlineKeyboardButton("📊 К статистике", callback_data="back_to_stats")])
...
async def invite_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Новый текст для кнопки "Поделиться"
    share_text = (
        f"💕 *Привет, подружка!* 💕\n\n"
...
async def cancel_share_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик отмены после создания теста"""
    query = update.callback_query
    ...
    try:
        # Пробуем удалить сообщение с кнопками
    ...
    # Очищаем временные данные создания теста
    if 'create_test' in context.user_data:
        del context.user_data['create_test']
...
def main():
    ...
    app.add_handler(CommandHandler("resetweekly", admin_reset_weekly_command))
